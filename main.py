        ...
        try:
            ...
            if result:
                labels = [
                    ...
                    ("专业", result["Major"]),
                    ("班主任", result["Teacher"]),
                    ("学生人数", result["NumberOfStudents"])
                ]
                for i, (label_text, value) in enumerate(labels):
                    Label(frame_result, text=f"{label_text}: {value}").grid(row=i + 2, column=0, columnspan=2, padx=10,
                                                                            pady=5)
                ...
                if result['Photo']:
                    ...
                    Label(frame_result, image=photo).grid(row=2, column=2, rowspan=9, padx=10, pady=5)
...
def delete_score():
    def del_score():
        try:
            ...
            if result['Score'] is not None:
                messagebox.showinfo("提示", "该课程已出成绩，无法退课")
                return
            else:
                cur.execute("SELECT StudentID, CourseID, Score FROM Grade WHERE StudentID = '%s' AND CourseID = %s" % (
                    entry_student_id.get(), entry_course_id.get()))
                if cur.fetchone() is None:
                    messagebox.showinfo("提示", "未找到相关记录")
                cur.execute("DELETE FROM Grade WHERE StudentID = '%s' AND CourseID = %s AND Score IS NULL" % (
                    entry_student_id.get(), entry_course_id.get()))
                db.commit()
    ...
    root_delete_score = Toplevel()
    root_delete_score.title("退课")
    root_delete_score.geometry("300x150")

    Label(root_delete_score, text="学生学号：").grid(row=0, column=0, padx=10, pady=5, sticky=E)
    entry_student_id = Entry(root_delete_score)
    entry_student_id.grid(row=0, column=1, padx=10, pady=5)

    Label(root_delete_score, text="课程ID：").grid(row=1, column=0, padx=10, pady=5, sticky=E)
    entry_course_id = Entry(root_delete_score)
    entry_course_id.grid(row=1, column=1, padx=10, pady=5)

    Button(root_delete_score, text="提交", command=del_score).grid(row=2, column=0, padx=10, pady=10)
    Button(root_delete_score, text="退出", command=root_delete_score.destroy).grid(row=2, column=1, padx=10, pady=10)
...
def create_score():
    ...
    button_del_score = Button(root2, text="退课", command=delete_score)
    button_del_score.place(x=200, y=400, height=40, width=200)
...
def change_course():
    def update_course():
        ...
        if entry_course_name.get() == "":
            entry_course_name.insert(0, result['CourseName'])
        if entry_teacher.get() == "":
            entry_teacher.insert(0, result['Teacher'])
        if entry_classroom.get() == "":
            entry_classroom.insert(0, result['Classroom'])
        try:
            cur.execute("UPDATE Course SET CourseName = '%s', Teacher = '%s', Classroom = '%s' WHERE CourseID = %s" %
                        (entry_course_name.get(), entry_teacher.get(), entry_classroom.get(),
